...
class Example extends FunSuite {
  ...
  test("FizzBuzz") {
    ...
    assert((fizzbuzz(n, Var("Fizz")) &&& Nat.toInt(n)).run.take(5) == Stream(3, 6, 9, 12, 18))
    assert((fizzbuzz(n, Var("Buzz")) &&& Nat.toInt(n)).run.take(5) == Stream(5, 10, 20, 25, 35))
    assert((fizzbuzz(n, Var("FizzBuzz")) &&& Nat.toInt(n)).run.take(5) == Stream(15, 30, 45, 60, 75))
    assert((Nat.nat(n) &&& Nat.lteq(Var(Nat(1)), n) &&& (fizzbuzz(n, s) &&& s.get ||| !fizzbuzz(n, s) &&& Nat.toInt(n).map(_.toString))).run.take(5) == Stream("1", "2", "Fizz", "4", "Buzz"))
  }
